...
//        return issueToken
//    }
//    // TODO: 수정 필요 - assignee api사용으로 수정 필요
//    func createAssigneeToken() -> [FilterToken<IssueEntity>] {
//        let title = "담당자"
//        let icon = "person"
//        let assigneeToken: [FilterToken<IssueEntity>] = issues.map { issue in
//            FilterToken(title: title, description: issue.user.login, icon: icon, conditon: { issue in issue.assignees.contains { user in user.login == issue.user.login } })}
...
//            FilterToken(title: title, description: "레이블 없음", icon: icon, conditon: { issue in issue.labels.isEmpty}),
//            FilterToken(title: title, description: "documentation", icon: icon, conditon: { issue in issue.labels.contains { label in label.name == "documentation" } }),
//            FilterToken(title: title, description: "bug", icon: icon, conditon: { issue in issue.labels.contains { label in label.name == "bug" } })
//        ]
//
//        return labelToken
//    }
